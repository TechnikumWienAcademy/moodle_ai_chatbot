...
$event = \mod_openaichat\event\course_module_instance_list_viewed::create(array(
    'context' => $coursecontext
));
$event->add_record_snapshot('course', $course);
$event->trigger();

$PAGE->set_url('/mod/openaichat/index.php', array('id' => $id));
...
if (empty($openaichats)) {
    notice(get_string('no$openaichatinstances', 'mod_openaichat'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
foreach ($openaichats as $openaichat) {
    if (!$openaichat->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/openaichat/view.php', array('id' => $openaichat->coursemodule)),
    ...
    if ($course->format == 'weeks' || $course->format == 'topics') {
        $table->data[] = array($openaichat->section, $link);
    } else {
